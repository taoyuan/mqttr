...
function Client(mqttclient) {
  ...
  mqttclient.on('message', function (topic, message) {
    that._handleMessage(topic, JSON.parse(message));
  });
}
...
Client.prototype._subscribe = function (topic, options, cb) {
  ...
  return options ? this.mqttclient.subscribe(mqttTopic(topic), options, cb)
    :this.mqttclient.subscribe(mqttTopic(topic), cb);
};
...
Client.prototype._handleMessage = function (topic, message) {
  var sub, matched = this.router.match(topic);
  if (!matched) {
...
Client.prototype.subscribe = function (topic, handler, options, cb) {
  ...
  if (this.connected) this._subscribe(topic, options, cb);
  return sub;
};

Client.prototype.publish = function (topic, message, options, cb) {
  this.mqttclient.publish(topic, JSON.stringify(message), options, cb);
};
